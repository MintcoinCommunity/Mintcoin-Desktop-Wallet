...
Value getnewpubkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"MintCoinaddress\"    (string) The new MintCoin address\n"
            "\nExamples:\n"
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
		...
		if((wtx.IsCoinBaseOrStake() && nDepth >= nMinDepth && wtx.GetBlocksToMaturity() == 0)
			|| !wtx.IsCoinBaseOrStake())
		{
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            "settxfee amount\n"
            "\nSet the transaction fee. 'amount' is a real and is rounded to the nearest 0.01\n"
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in MINT rounded to the nearest 0.01\n"
            "\nResult\n"
...
Value resendtx(const Array& params, bool fHelp)
{
    ...
    pwalletMain->ResendWalletTransactionsBefore(GetTime());

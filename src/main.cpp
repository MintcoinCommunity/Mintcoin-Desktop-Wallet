                        ...
                        bool* pfMissingInputs)
{
...

    {
        ...
        int64 txMinFee = GetMinFee(tx, 1000, true, GMF_RELAY, nSize);
...
static const int64 nTargetSpacingWorkMax = 3 * Params().StakeTargetSpacing(); 
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    int64 nTargetSpacing = fProofOfStake? Params().StakeTargetSpacing() : min(nTargetSpacingWorkMax, (int64) Params().StakeTargetSpacing() * (1 + pindexLast->nHeight - pindexPrev->nHeight));
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    printf("InvalidChainFound: invalid block=%s  height=%d  trust=%.8g  date=%s\n",
      pindexNew->GetBlockHash().ToString().c_str(), pindexNew->nHeight,
      log(pindexNew->nChainTrust.getdouble())/log(2.0), DateTimeStrFormat("%Y-%m-%dT%H:%M:%S",
      pindexNew->GetBlockTime()).c_str());
    printf("InvalidChainFound:  current best=%s  height=%d  trust=%.8g  date=%s\n",
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks, CBlock *block)
{
    if (!tx.IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins.IsCoinBase() || coins.IsCoinStake(prevout.hash))
            {
                if (nSpendHeight - coins.nHeight < nMinConfirmations)
                    return state.Invalid(error("CheckInputs() : tried to spend %s at depth %d",txPrev.IsCoinBase() ? "coinbase" : "coinstake", nSpendHeight - coins.nHeight));
            }
        ...
        if (tx.IsCoinStake())
        {
            ...
            int64 nStakeReward = GetValueOut(tx) - nValueIn;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for(unsigned int i=0; i<block.vtx.size(); i++)
    {
        ...
        SyncWithWallets(block.GetTxHash(i),tx, &block, false, false);
    }
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    nTransactionsUpdated++;
    printf("SetBestChain: new best=%s  height=%d  trust=%.8g  tx=%lu  date=%s  progress=%f\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.GetBlockTime() > (int64)block.vtx[0].nTime + GetClockDrift((int64)block.vtx[0].nTime))
        return state.DoS(50, error("CheckBlock() : coinbase timestamp is too early"));

    // Check coinstake timestamp
    if (block.IsProofOfStake() && !CheckCoinStakeTimestamp(block.GetBlockTime(), (int64)block.vtx[1].nTime))
    ...
    BOOST_FOREACH(const CTransaction& tx, block.vtx)
    {
        ...
        // ppcoin: check transaction timestamp
        if (block.GetBlockTime() < (int64)tx.nTime)
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
    {
        ...
        {
            return state.DoS(error("ProcessBlock() : block with too little %s", pblock->IsProofOfStake()? "proof-of-stake" : "proof-of-work"));
        }
    }
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            CValidationState state;
            if (!CheckInputs(tx, state, view, true, SCRIPT_VERIFY_P2SH, NULL, pblock.get()))
                continue;

...
int64_t GetMinFee(const CTransaction& tx, unsigned int nBytes, enum GetMinFee_mode mode)
{
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
...

    {
        ...
        int64_t txMinFee = GetMinFee(tx, nSize, GMF_RELAY);
    ...
    g_signals.SyncTransaction(tx, NULL, true);
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    BOOST_FOREACH(const CTransaction& tx, block.vtx)
        g_signals.SyncTransaction(tx, &block, true);
...
// Make chainMostTrust correspond to the chain with the most work in it, that isn't
// known to be invalid (it's however far from certain to be valid).
void static FindMostTrustChain() {
    CBlockIndex *pindexNew = NULL;

    // In case the current best is invalid, do not consider it.
    while (chainMostTrust.Tip() && (chainMostTrust.Tip()->nStatus & BLOCK_FAILED_MASK)) {
        setBlockIndexValid.erase(chainMostTrust.Tip());
        chainMostTrust.SetTip(chainMostTrust.Tip()->pprev);
    }
    ...
    if (chainMostTrust.Tip() && !CBlockIndexTrustComparator()(chainMostTrust.Tip(), pindexNew))
        return;

    // We have a new best.
    chainMostTrust.SetTip(pindexNew);
}
...
// Try to activate to the most-trust chain (thereby connecting it).
bool ActivateBestChain(CValidationState &state) {
    ...
    while (!fComplete) {
        ...
        if (chainMostTrust.Tip() == NULL) break;

        // Disconnect active blocks which are no longer in the best chain.
        while (chainActive.Tip() && !chainMostTrust.Contains(chainActive.Tip())) {
            if (!DisconnectTip(state))
                return false;
        }

        // Connect new blocks.
        while (!chainActive.Contains(chainMostTrust.Tip())) {
            CBlockIndex *pindexConnect = chainMostTrust[chainActive.Height() + 1];
            if (!ConnectTip(state, pindexConnect)) {
                if (state.IsInvalid()) {
                    // The block violates a consensus rule.
                    if (!state.CorruptionPossible())
                        InvalidChainFound(chainMostTrust.Tip());
                    fComplete = false;
...
CBlockIndex* AddToBlockIndex(CBlockHeader& block)
{
    ...
    CBlockIndex* pindexNew = new CBlockIndex((CBlock&)block);
    assert(pindexNew);
    {
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fProofOfStake, bool fCheckPOW)
{
    // Check proof of work matches claimed amount
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!CheckBlockHeader(block, state, block.IsProofOfStake(), fCheckPOW))
        return false;
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, bool fProofOfStake, CBlockIndex** ppindex)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash();
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        if (block.nBits != GetNextTargetRequired(pindexPrev, fProofOfStake))
            return state.DoS(100, error("AcceptBlock() : incorrect %s", !fProofOfStake ? "proof-of-work" : "proof-of-stake"),
                             REJECT_INVALID, "bad-diffbits");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (!AcceptBlockHeader(block, state, block.IsProofOfStake(), &pindex))
        return false;
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pblock->hashPrevBlock != 0 && (it == mapBlockIndex.end() || !(it->second->nStatus & BLOCK_HAVE_DATA)))
    {
        ...
        CBlock* pblock3 = new CBlock(*pblock);
        // ppcoin: check proof-of-stake
        if (pblock3->IsProofOfStake())
        {
            // Limited duplicity on stake: prevents block flood attack
            // Duplicate stake allowed only when there is orphan child block
            if (setStakeSeenOrphan.count(pblock3->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash) && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
                return error("ProcessBlock() : duplicate proof-of-stake (%s, %d) for orphan block %s", pblock3->GetProofOfStake().first.ToString().c_str(), pblock3->GetProofOfStake().second, hash.ToString().c_str());
            else
...
bool InitBlockIndex() {
    ...
    // Only add the genesis block if not reindexing (in which case we reuse the one already on disk)
    {
        try {
            ...
            CValidationState state;
            if (!FindBlockPos(state, blockPos, nBlockSize+8, 0, block.GetBlockTime(), fReindex))

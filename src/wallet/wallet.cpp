...
CFeeRate CWallet::minTxFee = CFeeRate(10 * CENT);  // Override with -mintxfee
...
CAmount CWallet::GetMintedBalance() const
{
    int64_t nTotal = 0;
    {
        LOCK(cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin.IsCoinStake() && IsFinalTx(pcoin) && pcoin.IsTrusted())
                nTotal += pcoin.GetValueOut()-GetDebit(pcoin, ISMINE_SPENDABLE);
        }
    }
    return nTotal;
}
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0 && nSubtractFeeFromAmount == 0 && nChange < MIN_TXOUT_AMOUNT)
                {
                    nFeeRet += nChange;
                    nChange = 0;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (newTxOut.IsDust(::minRelayTxFee) || nChange < MIN_TXOUT_AMOUNT)
                    {
                        CAmount nDust = newTxOut.GetDustThreshold(::minRelayTxFee) - newTxOut.nValue;
                        if (nDust < MIN_TXOUT_AMOUNT)
                            nDust = MIN_TXOUT_AMOUNT;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CTransaction& txNewIn)
{
    ...
    if(pIndex0->pprev)
        nCombineThreshold = GetProofOfWorkReward(pIndex0->nHeight, minTxFee.GetFeePerK(), pIndex0->pprev->GetBlockHash()) / 3;
    ...
    // printf(">> block.GetBlockTime() = %d, nStakeMinAge = %d, txNew.nTime = %d\n", block.GetBlockTime(), nStakeMinAge,txNew.nTime);
        ...
        if (CheckStakeKernelHash(nBits, block, block.GetTxOffset(tx), *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake))
        {
           // Found a kernel
            if (fDebug && GetBoolArg("-printcoinstake", false))
                ...
                // printf(">> Wallet: CreateCoinStake: nCredit = %d\n", nCredit);
    ...
    {
        // printf(">> Wallet: CreateCoinStake: nCredit = %d, nBalance = %d, nReserveBalance = %d\n", nCredit, nBalance, nReserveBalance);
        return false;
    }
    ...
    // Calculate coin age reward
    {
        ...
        if (!GetCoinAge(((CTransaction)txNew), nCoinAge))
    ...
    while (true)
    {
        ...
        if (nMinFee < minTxFee.GetFeePerK())
        {
            nMinFee = minTxFee.GetFeePerK();

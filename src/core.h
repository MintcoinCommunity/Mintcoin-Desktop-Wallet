...
static const CAmount CIRCULATION_MONEY = MAX_MONEY;
static const double TAX_PERCENTAGE = 0.01;
static const CAmount MAX_MINT_PROOF_OF_STAKE = 0.05 * COIN;	// 5% annual interest
 ...
 *  (coinbase or not, height, transaction version and timestamp)
 */
class CTxInUndo
{
public:
    ...
    CTxInUndo() : txout(), fCoinBase(false), fCoinStake(false), nHeight(0), nTime(0), nVersion(0) {}
    CTxInUndo(const CTxOut &txoutIn, bool fCoinBaseIn = false, bool fCoinStakeIn = false, unsigned int nHeightIn = 0, unsigned int nTimeIn = 0, int nVersionIn = 0) : txout(txoutIn), fCoinBase(fCoinBaseIn), fCoinStake(fCoinStakeIn), nHeight(nHeightIn), nTime(nTimeIn), nVersion(nVersionIn) { }
